// #define __LOG_ON 1
/* tty is a device used to detect user's input and output
* input come from the keyboard
...
/* receive char from kb_header, write to tty_buf */
...
int tty_read(struct inode *ip, char *dest, __uint32_t n){
    u_int32_t ar = n;
    char ch;

    assert(ip, "tty_read: null ip");

    printl("tty_read: inode-%d dest: 0x0%x len: %d \n", ip->ino, dest, n);
    while (n > 0){
        if (tty_buf.nread == tty_buf.nwrite){   // empty
            printl("tty_read: empty\n");
            if (proc->killed){
                pointl("tty_read: fail\n");
                return -1;
            }
            sleep(&tty_buf.nread);
        }
